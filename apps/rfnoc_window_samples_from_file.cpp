...
//
// Example application to show how to use RFNoC Window Block.
// Set coefficients, send samples from file to window block,
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    if (vm.count("help")) {
        std::cout << "RFNoC Window Block samples from file" << desc << std::endl;
        std::cout << std::endl
                  << "Application that sends samples from a file to a "
                  << "RFNoC Window block and writes received samples to a file.\n"
    ...
    // Verify we have a window block and instantiate block controller
    auto window_blocks = graph->find_blocks<uhd::rfnoc::window_block_control>("");
    if (window_blocks.empty()) {
        std::cout << "ERROR: No window block found." << std::endl;
        return EXIT_FAILURE;
    }
    auto window_block =
        graph->get_block<uhd::rfnoc::window_block_control>(window_blocks.front());
    if (!window_block) {
    ...
    // Connect blocks to create a Host -> Window Block -> Host flowgraph
    graph->connect(tx_stream, 0, window_block->get_block_id(), 0);
    graph->connect(window_block->get_block_id(), 0, rx_stream, 0);
    graph->commit();

    // Load coefficients from file
    auto max_num_coeffs = window_block->get_max_num_coefficients(0);
    ...
    if (num_coeffs > max_num_coeffs) {
        ...
        std::cout << "ERROR: Provided " << num_coeffs << " coefficients "
                  << "but the window block supports a maximum of "
    ...
    window_block->set_coefficients(coeffs_buff, 0);
    coeffs_ifs.close();
